/**
 * Formats a raw population number into a human-readable string.
 */
export function formatPopulation(number) {
  if (typeof number !== "number" || isNaN(number)) return "N/A";
  if (number >= 1_000_000_000) return `${(number / 1_000_000_000).toFixed(1)} Billion`;
  if (number >= 1_000_000)     return `${(number / 1_000_000).toFixed(1)} Million`;
  if (number >= 1_000)         return `${(number / 1_000).toFixed(1)} Thousand`;
  return number.toString();
}

const REGION_EMOJI = {
  Asia: "🌏", Europe: "🌍", Africa: "🌍", Americas: "🌎", Oceania: "🌏", Antarctic: "🧊",
};
 ...
 * world-atlas TopoJSON uses numeric IDs (e.g. "104" for Myanmar, "840" for USA).
 * This replaces the old Alpha-3 map.
 */
const NUMERIC_TO_ALPHA2 = {
  ...
  // Small/island nations and Kosovo added to match countries.json
  "020":"AD","028":"AG","048":"BH","052":"BB","132":"CV","308":"GD",
  "336":"VA","438":"LI","462":"MV","470":"MT","480":"MU","492":"MC",
  "659":"KN","662":"LC","670":"VC","674":"SM","678":"ST","690":"SC",
  "702":"SG","983":"XK",
};

/**
 * Convert a world-atlas numeric ISO code to Alpha-2.
 * @param {string|number} numericId  e.g. "104" or 104
 * @returns {string|null}
 */
...
/**
 * @deprecated world-atlas uses numeric IDs, not Alpha-3.
 * Kept for backward compat; delegates to numericToAlpha2.
 */
